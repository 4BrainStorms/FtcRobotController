...
@Autonomous(name = "RRPath + Limelight Align", group = "auton")
public class RunRRPathPlusLLAlign extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        // ----- Grab the RRPathGen path that begins at your D1 pose -----
        Pose2d[] path = PathLibraryRRPG.TRAJECTORYTEST_FROM_D1();
        Pose2d startPose = path[0];

        // Initialize drive at that exact start pose
        ...
        ll.pipelineSwitch(PIPELINE_ALLTAGS); // show we’re alive
        ll.start();

        // Choose alliance in INIT
        ...
        if (ffTuned) {
            ...
            // Open-loop: go waypoint by waypoint using a small P controller
            for (int i = 1; i < path.length; i++) {
                goToPoseOpenLoop(drive, path[i], 8000);
            }
        }

        // Switch to the goal pipeline for your alliance and yaw-align using tx
